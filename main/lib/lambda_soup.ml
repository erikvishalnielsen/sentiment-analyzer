...
let get_list_items contents : (string * string) list =
  ...
  let news_table =
    List.find_exn all_tables ~f:(fun table ->
      List.exists (classes table) ~f:(fun a -> String.equal a "news-table"))
  in
  let table_rows =
    news_table
    $$ "tr"
    |> to_list
    |> List.filter ~f:(fun tr ->
      List.exists (classes tr) ~f:(fun a -> String.equal a "has-label"))
  in
  List.map table_rows ~f:(fun li ->
    let tds = li $$ "td" |> to_list in
    match tds with
    | [ first; second ] ->
      ...
      let actual_name =
        List.find_exn
          (second $$ "a" |> to_list)
          ~f:(fun table ->
            List.exists (classes table) ~f:(fun a ->
              String.equal a "tab-link-news"))
      in
...
(* let%expect_test "web scraper" = print_s [%sexp (get_list_items
   (Curl.get_exn "https://finviz.com/quote.ashx?t=AMZN&p=d") : (string *
   string) list)]; [%expect {|true|}] ;; *)
